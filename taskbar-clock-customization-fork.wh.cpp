...
// @name            Taskbar Clock Customization — mzihadul Fork
// @description     Custom date/time format, news feed, weather, performance metrics, media player info, custom fonts and colors, microphone and volume indicators, and more
// @version         1.0.0
// @author          m417z, mzihadul
...
// Modified and maintained by: mzihadul
// Modification date: 2026-09-17
//
// The original work is licensed under the GNU General Public License v3.0.
// This modified version is also distributed under the GNU GPL v3.0.
// The original attribution and license notice have been retained.
//
...
(upload/download speed, CPU, GPU, RAM, battery), microphone and volume indicators, media player info, custom fonts
and colors, and more.
...
* System performance metrics:
  ...
  * `%battery%` - battery level percentage and when plugged in it will show the %/AC.
  ...
  * `%volume%` - show system volume status with icon. When changing the volume it will show the % and will auto hide later on.
  * `%mic%` - microphone active in-use status indicator (🎙️).
* Media player info (requires a
  ...
  * `%media_info%` - combined media info, truncated with ellipsis. Defaults to
    ...
    tooltip.
* `%weather%` - Weather information, powered by [wttr.in](http://wttr.in/),
...
std::atomic g_isBatteryLow{false};
...
PCWSTR GetBatteryFormatted() {
    return GetMetricFormatted(
        g_batteryFormatted, [](PWSTR buffer, size_t bufferSize) {
            ...
            // Plugged into AC: show Percentage + ⚡ (e.g., "94%⚡")
            if (powerStatus.ACLineStatus == 1) {
                if (powerStatus.BatteryLifePercent != 255) {
                    swprintf_s(buffer, bufferSize, L"%d%%⚡", powerStatus.BatteryLifePercent);
                } else {
            ...
            // On Battery
            if (powerStatus.BatteryLifePercent != 255) {
                // Low battery (20% or less): prefix with a bright red warning
...
PCWSTR GetVolumeFormatted() {
    // Stores previous state and timer to detect volume changes
    static int s_lastVolume = -1;
    static BOOL s_lastMuted = FALSE;
    static ULONGLONG s_showUntilTick = 0;
        ...
        if (SUCCEEDED(hr)) {
            ...
            if (SUCCEEDED(hr)) {
                ...
                if (SUCCEEDED(hr)) {
                    ...
                    int volumePercent = (int)((currentVolume * 100.0f) + 0.5f);
                    ULONGLONG now = GetTickCount64();

                    // First run: initialize without popping up the timer
                    if (s_lastVolume == -1) {
                        s_lastVolume = volumePercent;
                        s_lastMuted = isMuted;
                    } 
                    // State changed: trigger a 3-second display countdown
                    else if (s_lastVolume != volumePercent || s_lastMuted != isMuted) {
                        s_lastVolume = volumePercent;
                        s_lastMuted = isMuted;
                        s_showUntilTick = now + 3000; // 3000 ms = 3 seconds
                    }

                    PCWSTR icon = isMuted ? L"🔇" : L"🔊";

                    // Show percentage only if within the active change window
                    if (now < s_showUntilTick) {
                        swprintf_s(g_volumeFormatted.buffer, L"%s %d%%", icon, volumePercent);
                    } else {
                        swprintf_s(g_volumeFormatted.buffer, L"%s", icon);
                    }
...
bool IsMicrophoneInUse() {
    static const PCWSTR kConsentPaths[] = {
    ...
    for (PCWSTR path : kConsentPaths) {
        HKEY hBaseKey;
        if (RegOpenKeyExW(HKEY_CURRENT_USER, path, 0, KEY_READ, &hBaseKey) == ERROR_SUCCESS) {
            DWORD subKeyIndex = 0;
            WCHAR subKeyName[256];
            DWORD subKeyNameLen = ARRAYSIZE(subKeyName);

            while (RegEnumKeyExW(hBaseKey, subKeyIndex++, subKeyName, &subKeyNameLen,
                                 nullptr, nullptr, nullptr, nullptr) == ERROR_SUCCESS) {
                HKEY hSubKey;
                if (RegOpenKeyExW(hBaseKey, subKeyName, 0, KEY_READ, &hSubKey) == ERROR_SUCCESS) {
                    ULONGLONG stopTime = 0;
                    DWORD size = sizeof(stopTime);
                    DWORD type = 0;

                    LONG res = RegQueryValueExW(hSubKey, L"LastUsedTimeStop", nullptr,
                                                &type, (LPBYTE)&stopTime, &size);
                    RegCloseKey(hSubKey);

                    // When actively in use, LastUsedTimeStop is 0
                    if (res == ERROR_SUCCESS && stopTime == 0) {
...
PCWSTR GetMicFormatted() {
    if (g_micFormatted.formatIndex != g_formatIndex) {
        if (IsMicrophoneInUse()) {
            wcscpy_s(g_micFormatted.buffer, L"🎙️");
        } else {
            wcscpy_s(g_micFormatted.buffer, L"");
        }
...
size_t ResolveFormatToken(
    ...
    struct {
        ...
        {L"%volume%"sv, GetVolumeFormatted},
        {L"%mic%"sv, GetMicFormatted},
...
void ApplyTextBlockStyles(
    ...
    } else if (textStyleSettings && *textStyleSettings->textColor) {
